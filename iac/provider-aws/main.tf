...
module "cluster" {
  ...
  cluster_secret_arn              = module.init.cluster.secret_arn
...
module "nomad" {
  ...
  # Upstream retired these module inputs (added in e2b-dev/infra#2081, later
  # removed); the root variables remain declared but are no longer plumbed to
  # the nomad module. Passing them fails validation.
