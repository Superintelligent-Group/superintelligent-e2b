...
- `main.tf` – VPC + subnets, NAT, S3 artifact buckets, ECR repos, ALB + HTTPS listener, DNS records (Route 53 or Cloudflare), and a worker IAM role with access to the buckets/ECR/CloudWatch.
...
If you use Cloudflare instead of Route 53, set `cloudflare_api_token` + `cloudflare_zone_id` and leave `create_route53_record=false`.
...
- **User data / service bootstrap:** The EC2 launch templates accept `control_plane_user_data` and `worker_user_data`, but the defaults are empty. Provide cloud-init/systemd scripts that pull artifacts, register with Nomad/Consul, and start the E2B services.
