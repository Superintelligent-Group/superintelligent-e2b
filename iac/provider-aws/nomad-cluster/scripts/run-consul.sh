...
function generate_consul_config {
  ...
  if [[ -z "$cluster_tag_name" ]] || [[ -z "$cluster_tag_value" ]]; then
    log_warn "The --cluster-tag-name or --cluster-tag-value property is empty. Will not automatically try to form a cluster based on EC2 tags."
  else
    retry_join_json=$(
      cat <<EOF
"retry_join": ["provider=aws tag_key=$cluster_tag_name tag_value=$cluster_tag_value"],
EOF
    )
  fi
