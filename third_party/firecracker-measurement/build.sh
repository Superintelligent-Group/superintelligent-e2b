...
# credentials, Docker socket, KVM device, or provider destination is passed in.
mkdir /work
cd /work
...
lanes = {}
for name in ('writer-tests', 'legacy-metrics-tests', 'action-tests', 'metrics-api-tests', 'response-tests'):
    ...
    lanes[name] = int(counts[0])
pathlib.Path('/output/validation.json').write_text(json.dumps({
    'rust': '1.89.0', 'target': 'x86_64-unknown-linux-musl',
    'focused_tests': lanes, 'format': 'passed', 'release_build': 'passed',
    'kvm_tests': 'not run', 'complete_measurement_window': False,
}, indent=2) + '\n')
PY
