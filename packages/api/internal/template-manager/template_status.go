...
var (
	...
	// Keep the build alive through a short outage in the status RPC. The builder
	// continues working while the API poller retries.
...
func (c *PollBuildStatus) poll(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				// A poll can finish at the same time as the build window. Do not
				// turn that cancellation into a false terminal build failure.
				if ctx.Err() != nil {
				...
				c.logger.Error(ctx, "Build status polling received unrecoverable error", zap.Error(failure))

				c.setFailed(ctx, fmt.Sprintf("polling received unrecoverable error: %s", failure))
