...
const fixture = path.join(root, 'packages/shared/pkg/storage/testdata/custody-model.csv')
const dafny = process.env.DAFNY || 'dafny'
if (process.argv.slice(2).some(arg => arg !== '--write')) throw new Error('usage: network-custody-model.mjs [--write]')
try {
  copyFileSync(path.join(root, 'specs/network-usage-custody.dfy'), path.join(temporary, 'model.dfy'))
  ...
  const rows = output.split(/\r?\n/).filter(line => /^(true|false),(true|false),(true|false),(true|false),(true|false)$/.test(line))
  if (rows.length !== 16) throw new Error(`Dafny emitted no oracle rows: ${output}`)
  const csv = rows.join('\n') + '\n'
