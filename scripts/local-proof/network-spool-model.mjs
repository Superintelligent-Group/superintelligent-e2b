...
const fixture = path.join(root, 'packages/orchestrator/pkg/sandbox/networkusage/testdata/spool-model.csv')
const dafny = process.env.DAFNY || 'dafny'
if (process.argv.slice(2).some(arg => arg !== '--write')) throw new Error('usage: network-spool-model.mjs [--write]')
try {
  copyFileSync(path.join(root, 'specs/network-usage-spool.dfy'), path.join(temporary, 'model.dfy'))
  ...
  const rows = output.split(/\r?\n/).filter(line => /^\d+,\d+,\d+,(true|false),(true|false)$/.test(line))
  if (rows.length !== 480) throw new Error(`Dafny emitted no oracle rows: ${output}`)
  const csv = rows.join('\n') + '\n'
